...
<div class="action-banner">
  ...
  <div class="text">
    ...
    with col1:
        ...
        st.caption("Long format 권장: 컬럼 2개 (부분군, 측정값)")
        uploaded = st.file_uploader("CSV 파일", type=['csv'], label_visibility='collapsed')
        if uploaded:
            # 같은 파일을 다시 처리(rerun)하지 않도록 파일명+크기로 식별
            file_key = f"{uploaded.name}_{uploaded.size}"
            already_loaded = (
                st.session_state.dataset is not None
                and st.session_state.dataset.get('meta', {}).get('_file_key') == file_key
            )

            df = pd.read_csv(uploaded)

            # Wide format 추정: 컬럼이 3개 이상이고 모두 숫자형일 때만 후보로 간주.
            # (Long format은 보통 [부분군, 측정값] 2개 컬럼이므로 자동 melt 대상에서 제외)
            looks_wide = (
                len(df.columns) >= 3
                and all(pd.api.types.is_numeric_dtype(df[c]) for c in df.columns)
            )
            do_melt = False
            if looks_wide:
                do_melt = st.checkbox(f"Wide format으로 감지됨 ({len(df.columns)}개 컬럼) — Long으로 변환?",
                                      value=False, key=f"melt_{file_key}")
                if do_melt:
                    df = df.melt(var_name='subgroup', value_name='value')
            elif len(df.columns) == 2:
                st.caption(f"✅ Long format으로 인식: `{df.columns[0]}`(부분군) · `{df.columns[1]}`(측정값)")

            # 최초 업로드(또는 melt 옵션이 바뀐 경우)에만 새 데이터셋 구성 — 불필요한 rerun/초기화 방지
            melt_key = f"{file_key}_{do_melt}"
            if st.session_state.get('_loaded_file_key') != melt_key:
                ...
                ds = {
                    ...
                    # attribute 모드로 전환할 때를 대비해 기본 후보를 미리 채워둠 (없으면 설정 UI가 안 뜸)
                    'size_col': num_cols[0] if len(num_cols) > 0 else None,
                    'defect_col': num_cols[1] if len(num_cols) > 1 else (num_cols[0] if num_cols else None),
                    'LSL': None, 'USL': None, 'target': None,
                    'meta': {'source': uploaded.name, 'description': '', '_file_key': file_key},
                }
                st.session_state.dataset = ds
                st.session_state.saved_dataset = {**ds, 'df': df.copy()}
                st.session_state['_loaded_file_key'] = melt_key
                st.success("✅ 업로드 완료")
                st.rerun()
    ...
    if st.session_state.dataset is not None:
        ...
        if d['mode'] == 'variable':
            ...
            if not num_options:
                st.warning("⚠️ 숫자형 컬럼이 없어 표본크기/불량 컬럼을 지정할 수 없습니다.")
            else:
                # size_col / defect_col 키가 없거나 유효하지 않으면 기본값으로 보정 (설정 UI가 사라지는 것 방지)
                if d.get('size_col') not in num_options:
                    d['size_col'] = num_options[0]
                if d.get('defect_col') not in num_options:
                    d['defect_col'] = num_options[1] if len(num_options) > 1 else num_options[0]
